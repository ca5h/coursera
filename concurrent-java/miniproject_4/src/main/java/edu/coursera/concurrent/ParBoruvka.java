...
public final class ParBoruvka extends AbstractBoruvka<ParBoruvka.ParComponent> {
    ...
    public void computeBoruvka(final Queue<ParComponent> nodesLoaded,
        ...
        while((node = nodesLoaded.poll()) != null) {
            //safe to parallelize 2 iterations working on disjoint (n, other) pairs if work list is thread-safe
            //poll removes 1-st el (node) from nodesLoaded work-list
            ...
            if (e == null) {
                ...
                break; //done - graph is contracted to single node
            }
            ...
            //merge node "other" to current node
            node.merge(other, e.weight());

            node.lock.unlock();
            other.lock.unlock();

            //add newly merged node to work list
            nodesLoaded.add(node);
        }
    }
