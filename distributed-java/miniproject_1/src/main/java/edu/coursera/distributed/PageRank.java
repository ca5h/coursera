...
public final class PageRank {
    ...
    public static JavaPairRDD<Integer, Double> sparkPageRank(
        ...
        /*
        For each website, iterate over its inbound edges and
        generate from the current website to the new rank of the target of that edge
        (site.getRank() / (double) site.getNEdges())
        The output newRanks is an RDD of Tuple2 key-value pairs,
        where the key is a unique ID for website
        and value is some contribution to that site's new rank from some other site that links to it.
         */
